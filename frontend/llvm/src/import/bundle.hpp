...
private:
  ...
  /// \brief Map a libm declaration onto the matching AR float intrinsic.
  ///
  /// Returns nullptr if `fun` is not a recognised libm entry point whose
  /// signature matches the operation.
  ///
  /// Needed because whether math.h reaches the frontend as an LLVM intrinsic or
  /// as a plain library call is a property of the compilation flags, not of the
  /// source. See the call site in translate_extern_function.
  ar::Function* translate_libm_intrinsic(llvm::Function*);
